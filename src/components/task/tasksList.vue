                ...
                <template v-slot:body="{ items }">
                  <tbody :key="item.id" v-for="item in items">
                    <tr :class="item.state">
                      ...
                      <td>
                        <span v-if="item.startTask !== ''">{{
                          dateFormat(item.updated_at)
                        }}</span>
                        <span v-else
                          ><v-btn
                            @click="start(item.id, null, 'general')"
                            icon
                            slot="activator"
                            text
                            ><v-icon>play_circle_filled</v-icon></v-btn
                          ></span
                        >
                      </td>
                      <td>{{ item.state }}</td>
    ...
    stop(generalTaskID, subTaskID, type) {
      ...
      if (type == 'general') {
        this.$store.dispatch('fetchTasksByID', generalTaskID).then(response => {
          ...
          vue.$store
            .dispatch('putTasks', response.data)
            .catch(error => {
              vue.$swal({
